...
namespace rageam::asset
{
	...
	struct MaterialTune : IXml
	{
		...
		MaterialTune(ConstString name, u16 sceneMaterialIndex, ConstString shaderName = DEFAULT_SHADER)
		{
			Name = name;
			Shader = shaderName;
			...
			SceneMaterialIndex = sceneMaterialIndex;
		}
	...
	struct DrawableAssetMap
	{
		...
		SmallList<u16>			SceneNodeToLightAttr;	// graphics::SceneLight		-> rage::CLightAttr
		...
		SmallList<u16>			LightAttrToSceneNode;	// rage::CLightAttr			-> graphics::SceneLight

		rage::grmModel* GetModelFromScene(rage::rmcDrawable* drawable, u16 sceneNodeIndex);
		rage::crBoneData* GetBoneFromScene(const rage::rmcDrawable* drawable, u16 sceneNodeIndex);
		rage::phBound* GetBoundFromScene(const gtaDrawable* drawable, u16 sceneNodeIndex);
		CLightAttr* GetLightFromScene(gtaDrawable* drawable, u16 sceneNodeIndex);
	};
	...
	class DrawableAsset : public GameRscAsset<gtaDrawable>
	{
		...
		void CreateAndSetCompositeBound();
